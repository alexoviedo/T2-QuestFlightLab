...
## v0.7 Procedural Scenic Splat Patch

v0.7 adds an opt-in procedural scenic splat patch for playable-demo approach background context. The patch is generated by `tools/generate_scenic_splat_patch.py` and contains stylized foothills, field color, tree-line, and airport-edge masses. It is synthetic/project-owned, not a real KBDU capture, not surveyed scenery, and not for navigation.

The mesh airport, runway markings, pattern gates, approach gates, runway collision path, cockpit, UI, and training logic remain mesh/generated objects. The splat patch is visual-only and fallback-safe.
