...
1. Integrate one small opt-in procedural scenic patch into the playable demo.
2. Keep mesh/terrain fallback as the default shipping path.
3. Capture Quest runtime timing for `scenic_splat_low`, `scenic_splat_medium`, and only then `scenic_splat_high`.
4. Capture or source a tiny owned/licensed real-world SPZ/PLY sample after the procedural playable path is stable.
5. Convert/import future owned captures into the current Unity renderer path without committing large raw assets.
6. Add explicit LOD/chunking/global-budget controls before increasing beyond 100k.
7. Investigate SPZ compression/conversion as an asset-pipeline step.

## v0.7 Playable Scenic Patch Direction

v0.7 uses a procedural/project-owned scenic patch instead of a public or Meta sample. The asset is generated by `tools/generate_scenic_splat_patch.py` and imported into these optional runtime modes:

| Mode | Splats | Role |
| --- | ---: | --- |
| `scenic_splat_low` | 25,000 | First playable scenic layer. |
| `scenic_splat_medium` | 50,000 | Main scenic patch candidate. |
| `scenic_splat_high` | 100,000 | Optional upper check if low/medium are healthy. |

This is still not a full-airport splat. It is a visual-only approach/background layer over the existing mesh airport.
